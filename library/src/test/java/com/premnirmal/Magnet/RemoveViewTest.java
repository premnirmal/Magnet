...
@PrepareForTest( { RemoveView.class, LayoutInflater.class } )
public class RemoveViewTest {
    ...
    public void setUp() throws Exception {
        ...
        whenNew(SimpleAnimator.class).withArguments(buttonMock, R.anim.slide_up).thenReturn(simpleAnimatorMockShow);
        whenNew(SimpleAnimator.class).withArguments(buttonMock, R.anim.slide_down).thenReturn(simpleAnimatorMockHide);
        whenNew(SimpleAnimator.class).withArguments(shadowMock, android.R.anim.fade_in).thenReturn(simpleAnimatorMockFadeIn);
    ...
    @Test
    public void testOnMoveXBiggerThanZero() {
        // given
    ...
    @Test
    public void testOnMoveXSmallerThanZero() {
        // given
        DisplayMetrics displayMetricsMock = mock(DisplayMetrics.class);
        Resources resourcesMock = mock(Resources.class);
        doReturn(contextMock).when(buttonMock).getContext();
    ...
    public void testDestroy() {
        // given
        assertNotNull("mLayout field musn't be null before destroy method call", getInternalState(removeView, "mLayout"));
        assertNotNull("mWindowManager field musn't be null before destroy method call", getInternalState(removeView, "mWindowManager"));

        // when
        removeView.destroy();

        // then
        assertNull("mLayout field must be set to null after destroy method call", getInternalState(removeView, "mLayout"));
        assertNull("mWindowManager field must be set to null after destroy method call", getInternalState(removeView, "mWindowManager"));
    }
}
